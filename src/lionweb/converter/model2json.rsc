...
SerializationChunk ast2jsonmodel(node astRoot, LionSpace lionspace, Language language) {
    ...
    // Find Lionweb type of the Rascal AST node
    ...
    Node instantiateNode(node astNode, Classifier lionType, Language language, LionSpace lionspace, Id parentId) {
        ...
        if (jsonNode.id in builtNodes) throw "The node of type <lionType.name> does not have unique id: <jsonNode.id>";
    ...
    value lionValue2json(value val, lionweb::m3::lioncore::Property property, Language language, LionSpace lionspace) {
        ...
        if (DataType(Enumeration enum) := valueType) {
            // val is the name of constructor of the format "plus()"
            str literalName = substring("<val>", 0, size("<val>") - 2);
            if ([*L1, literal: EnumerationLiteral(name = literalName), *L2] := enum.literals)
                return literal.key;         
            else throw "Cannot find <literalName> in <enum>";    
        } else
            return val;
    }
    ...
    // Main body of the transformation:
